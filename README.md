...
### Documentation
For a detailed bill of core materials, consult this [document](bom.pdf).

Any enclosure may be used, such as the [aluminium console](https://www.digikey.com/en/products/detail/lmb-heeger-inc/MDC752-PLAIN/1738) which was chosen for the initial prototype.
...
### Material Motivation
The circuit was largely butchered from the [Daisy Petal schematic](https://github.com/electro-smith/Hardware/blob/master/reference/daisy_petal/ES_Daisy_Petal_Rev5.pdf) with most of the circuit design and components being the same, though it is heavily simplified and uses breadboard compatible components as opposed to TSSOP. This was mostly done as I had never designed an audio sensitive analog circuit like this before. With regards to casing and the footswitch, those were recommendations given by my Professor (thanks Wendy!)

The goal was to use as much established literature as I could in order to focus on actually building and refinining the pedal (as it turns out, analog electrical issues are hard and took up all my time).

## Major components

### Input Buffer
The first core component of the project is the input buffer circuit, which effectively allows us to take a high impedance signal and transform it into a low impedance signal. This is important as the pickups of any electric guitar are high impedance and do not allow much current to flow through them. Using the Op-Amps of the MCP6002 IC, we can both bias the input to a specific voltage and ensure that signal is unity gain. This results in a cleaner, undistorted signal.

<img src="media/Input_Buffer_Circuit.jpg" alt="drawing" width="400"/>

### Output Buffer
Similarly to the input buffer, the output buffer circuit ensures our signal remains at unity gain. In this case, we use an Op-Amp designed to handle higher voltage signals and spikes specifically for audio, allowing the circuit to handle effects like high-gain distortion.
...
### Enclosure
Points for mounting holes were determined via cross-section as shown in the photo below. For the V1 prototype, a drill with  tips matched to the component dimensions was used to create mounting holes on both the top half of the enclosure and the bottom half. Finally, the enclosure was brushed with sandpaper.
...
### Daisy Seed
The Daisy Seed is a microcontroller specifically designed for audio processing and synthesis. It comes packaged with a variety of components needed for audio applications, including multiple Analog-Digital Converters, Digital-Analog Converters, (stereo) audio input/output and a fast M7 microprocessor with enourmous cache allowing for memory heavy effects such as complex *delay*. There are also an array of libraries included with the platform to both program the board and conduct DSP on analog input signals using C++. See [PedalBoi.cpp](PedalBoi.cpp) for distortion implementation details.
...
## Final Build
The final build comprised of the modified aluminium console containing the breadboard prototypes and 3PDT Switch, secured with 2 screws at the bottom of the console. Any 13-18V DC power source can be used to power the pedal via a positive and negative lead extending outside the case.
...
### Ground loops
Ground loops caused by different AC power sources were a substantial source of noise in the initial design of the pedal. It was tough to analyse and debug where the noise was actually coming from. Eventually, the entire circuit had to be redesigned with the help of an Oscilloscope to weed out the hum.
